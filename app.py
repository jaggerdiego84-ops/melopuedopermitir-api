...
def generar_pdf():
    try:
        ...
        return send_file(
            ...
            as_attachment=True,
            download_name=f"informe-melopuedopermitir.pdf"
        )
...
def webhook_stripe():
    ...
    print(f"==> Webhook recibido, sig: {sig_header[:20] if sig_header else 'NONE'}")
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        if email:
            try:
                print(f"==> Generando PDF para {email}...")
                pdf_bytes = generate_pdf(user_data)
                pdf_b64   = base64.b64encode(pdf_bytes).decode('utf-8')
                filename  = f"informe-melopuedopermitir.pdf"

                print(f"==> PDF generado ({len(pdf_bytes)} bytes). Enviando email...")
            ...
            print(f"==> ERROR: no hay email del cliente")
