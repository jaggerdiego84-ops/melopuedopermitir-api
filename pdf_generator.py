...
def generar_distribucion(ingresos, gastos, tl, margen, ratio, cmeses, ns, gastos_mes, loc):
    ...
    vivienda=gastos.get('vivienda',0)
    candidatos=[]
    ...
    pct_viv    = vivienda/ingresos*100 if ingresos>0 else 0
    ahorro_ideal = round(ingresos*0.15)

    # 1. Ocio
    recorte=round(ocio*0.22) if ocio>0 else round(ingresos*0.02)
    candidatos.append({'titulo':u'Auditar el ocio y las suscripciones','ahorro':recorte,'prioridad':1 if pct_ocio>10 else 3,
        'motivo':u'El ocio y las suscripciones son la categoría donde más margen se esconde sin notarlo. Representa el '+str(round(pct_ocio))+u'% de tus ingresos. Un recorte del 22% equivale a '+str(recorte)+u' '+EUR+u'/mes sin cambiar la calidad de vida: auditar suscripciones activas que no se usan (media de 4-5 por persona en España), cocinar en casa 2 veces más por semana, y sustituir una salida mensual por una alternativa de bajo coste. El efecto en 12 meses: '+str(recorte*12)+u' '+EUR+u' adicionales que pueden ir directamente al colchón de seguridad, equivalente a '+str(round(recorte*12/max(gastos_mes,1),1))+u' meses extra de cobertura.'})

    # 2. Ahorro sistematico
    extra=max(ahorro_ideal-ahorro_act, round(ingresos*0.03))
    meses_ns=round((ns-cmeses*gastos_mes)/ahorro_ideal) if ahorro_ideal>0 and cmeses<3 else 0
    candidatos.append({'titulo':u'Automatizar el ahorro al 15% el día de cobro','ahorro':-extra,'prioridad':1 if pct_ahorro<12 else 4,
        'motivo':u'Ahora destinas el '+str(round(pct_ahorro))+u'% al ahorro. El Banco de España recomienda el 15% como mínimo para construir seguridad financiera real. La diferencia son '+str(extra)+u' '+EUR+u' más al mes, que se convierten en invisibles si se domicilian automáticamente el mismo día de cobro, antes de que el dinero llegue a la cuenta corriente. El efecto compuesto es doble: llegas antes al colchón mínimo'+(u' (en '+str(meses_ns)+u' meses con este ritmo)' if meses_ns>0 else u'')+u' y cambias tu relación con el dinero porque empiezas a pagarte primero a ti mismo, que es el principio más sólido de la educación financiera personal.'})

    # 3. Transporte
    ahorro_t=round(transporte*0.15) if transporte>0 else round(ingresos*0.01)
    candidatos.append({'titulo':u'Reducir el coste de transporte un 15%','ahorro':ahorro_t,'prioridad':1 if pct_trans>7 else 5,
        'motivo':u'El transporte supone el '+str(round(pct_trans))+u'% de tus ingresos. Hay tres palancas que no requieren cambiar hábitos: revisar el seguro del coche cada año sin cambiar cobertura ahorra de media un 18% (entre 80 y 150 '+EUR+u' anuales); usar la app Geoportal Carburantes de la CNMC para localizar las gasolineras más baratas de '+loc+u' ahorra entre 8 y 15 '+EUR+u'/mes; y valorar el transporte público para trayectos cortos y recurrentes. Resultado potencial: '+str(ahorro_t)+u' '+EUR+u'/mes liberados sin impacto real en la movilidad diaria.'})

    # 4. Deudas
    if deudas>0:
        candidatos.append({'titulo':u'Estrategia de bola de nieve para las deudas','ahorro':0,'prioridad':1 if ratio>48 else 4,
            'motivo':u'Con '+str(round(deudas))+u' '+EUR+u'/mes en cuotas y un ratio del '+str(round(ratio))+u'%, cada deuda que terminas mejora el ratio directamente. La estrategia de bola de nieve consiste en concentrar el esfuerzo extra en la deuda con el tipo de interés más alto primero. Incluso 50-100 '+EUR+u'/mes de amortización anticipada puede adelantar el fin varios meses, ahorrando intereses equivalentes a 1-2 cuotas completas. Cuando termine la primera deuda, destina esa cuota íntegra a atacar la siguiente: es el momento donde el efecto se multiplica y la sensación de control financiero cambia radicalmente.'})

    # 5. Colchon si ya todo bien
    colchon_obj = round(gastos_mes*6)
    falta_colchon = max(colchon_obj - round(cmeses*gastos_mes), 0)
    aporte_colchon = round(margen*0.4) if margen>0 else round(ingresos*0.05)
    meses_colchon = round(falta_colchon/aporte_colchon) if aporte_colchon>0 and falta_colchon>0 else 0
    candidatos.append({'titulo':u'Construir el colchón de 6 meses','ahorro':-aporte_colchon,'prioridad':2 if cmeses<3 else 5,
        'motivo':u'El colchón de emergencia es la base de cualquier finanza sana. Ahora tienes '+str(round(cmeses,1))+u' meses cubiertos; el objetivo mínimo son 3, el óptimo son 6 ('+str(round(colchon_obj)).replace("'","")+u' '+EUR+u'). Destinar '+str(aporte_colchon)+u' '+EUR+u'/mes al colchón'+(u' te permitiría llegar al objetivo en '+str(meses_colchon)+u' meses' if meses_colchon>0 else u' acelera el proceso')+u'. Con 6 meses cubiertos, cualquier imprevisto — pérdida de empleo, avería grande, gasto médico — deja de ser una crisis y se convierte en una decisión tranquila que puedes tomar sin presión.'})

    # Ordenar por prioridad y devolver los 3 mejores
